...
%		OPTIONS.tau2 : Left bracket reduction used in section phase,default 0.1.
...
defaultopt = struct('Display','iter','HessUpdate','bfgs','GoalsExactAchieve',0,'GradConstr',false,  ...
			'TolX',1e-16,'TolFun',1e-2,'GradObj','on','MaxIter',400,'MaxFunEvals',100*numel(x_init)-1,  ...
			'DiffMaxChange',1e-1,'DiffMinChange',1e-10,'OutputFcn',[], ...
			'rho',0.0100,'sigma',0.9000,'tau1',4,'tau2', 0.1, 'tau3', 0.9,'StoreN',50);
...
data.ngrad = norm(grad);
data.gNorm = norm(data.gradient,Inf);
...
%   data
  if isfield(data,'section_exitflag')
...
%     itw
%     (log(optim.TolFun)/log(optim.tau3))
    ...
    if(data.beta>0&&hill)
            ...
            if(optim.GradConstr)
                gstep=data.initialStepLength/1e31; 
